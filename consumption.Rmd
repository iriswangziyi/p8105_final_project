...
## Consumption
This graph shows data from the Per Capita Consumption by state from year 2010-2019. This consumption is by per capita and only considers individuals who are 21 years old or older. The colors represent the different regions of each state.  
...
intro to the below map

```{r brewer map, message = F, warning = F, cache = T} 
...
beer %>% 
  ...
  labs(
    ...
    #subtitle = "Data from years ",
    y = "ABV content",
    x = "State"
  )
```

## Most common style by state
```{r}
top_styles = beer %>% 
  filter(state %in% c(state.name)) %>%
  drop_na(style) %>%
 ...
 DT::datatable(., rownames = F, 
  ...
  DT::formatStyle(
      ...
      backgroundColor = DT::styleEqual(
        unique(top_styles$style), c('lightsteelblue','lightcyan', 'lightblue', 'lightskyblue',
                                    'cornflowerblue', 'skyblue', 'paleturquoise', 'powderblue' )
      )
  )
