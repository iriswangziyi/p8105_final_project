...
This graph looks at the amount of Barrels produced by state recorded in barrel units. The points represents the mean barrel production from 2010-2019 and the lines represent the deviation of the data. The data comes from the Alcohol and Tobacco Tax and Trade Bureau data set. It helps us see the difference in can and bottle production by state. The colors represent the states region, the options are "Northeast", "South", "North Central" and "West". 
...
tax_cans_df %>% # first we need to pivot the data 
  filter(state != "total") %>%
  pivot_longer(
    c(`2008`:`2019`),
    names_to = "year",
  ...
  group_by(state) %>%
  summarize(
    mean_barrel = mean(barrels, na.rm = TRUE), # finding the mean barrels
    CI_lower = mean(barrels, na.rm = TRUE) - 
      1.96 * sd(barrels, na.rm = TRUE)/sqrt(length(barrels)), # finding the lower CI
    CI_upper = mean(barrels, na.rm = TRUE) + 
      1.96 * sd(barrels,na.rm = TRUE)/sqrt(length(barrels)) # finding the upper CI
  ) %>%
...
This graph looks at the amount of cans and bottles produced by state recorded in barrel units. The points represents the mean bottle production from 2010-2019 and the lines represent the deviation of the data.  The data comes from the Alcohol and Tobacco Tax and Trade Bureau data set. It helps us see the difference in can and bottle production by state. The colors represent the states region, the options are "Northeast", "South", "North Central" and "West". 
...
can_df = right_join(longer_can_df, region, by = "state")
...
### Bottles and Cans Analysis: 

--- 

## Shipments of Malt Beverages
This is the amount of shipments of the United States has made of malt beverages in barrel units. The moving picture shows years from 2010 to 2019 and will continuously repeat.  This data comes from the American Beer Distributors. 
